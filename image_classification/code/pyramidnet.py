...
class PyramidNet(nn.Module):
    ...
    def __init__(self, dataset, depth, alpha, num_classes, bottleneck=False):
        ...
        if self.dataset.startswith('cifar'):
            ...
            self.uncertainty = nn.Linear(self.final_featuremap_dim, 128)
    ...
    def forward(self, x, signal='train'):
        if self.dataset == 'cifar10' or self.dataset == 'cifar100':
            ...
            x = self.avgpool(x)
            x = x.view(x.size(0), -1)
            x_uncertainty = self.uncertainty(x)
            x_uncertainty = torch.nn.functional.relu(x_uncertainty)
            p = self.fc(torch.eye(x.size()[1]).cuda())
            p_uncertainty = torch.zeros(128, self.num_classes).cuda()
            p = p.t()
            p_uncertainty = p_uncertainty.t()
            if signal == 'test':
                x = self.fc(x)
            else:
                x_ori = self.fc(x)
                sq_x = torch.sum(torch.pow(x, 2), 1, keepdim=True).expand(x.size()[0], p.size()[0])
                sq_p = torch.sum(torch.pow(p, 2), 1).expand(x.size()[0], p.size()[0])
                distance = sq_x + sq_p - 2*torch.mm(x, p.t())
                sq_x_uncertainty = torch.sum(torch.pow(x_uncertainty, 2), 1, keepdim=True).expand(x.size()[0], p.size()[0])
                sq_p_uncertainty = torch.sum(torch.pow(p_uncertainty, 2), 1).expand(x.size()[0], p.size()[0])
                distance_uncertainty = sq_x_uncertainty + sq_p_uncertainty - 2*torch.mm(x_uncertainty, p_uncertainty.t())
                x = 1 - torch.mul((1-x_ori), torch.exp(-torch.div(distance_uncertainty, 5*distance)))
